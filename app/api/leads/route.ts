// API Route: Create Lead & Generate Intake Link
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { commissioner_id, client_name, client_phone, client_email, project_summary, budget } = body;

        // Validate required fields
        if (!commissioner_id || !client_name) {
            return NextResponse.json(
                { error: 'Commissioner ID and client name are required' },
                { status: 400 }
            );
        }
